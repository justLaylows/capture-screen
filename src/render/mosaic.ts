import { RENDERINFO } from '../types/index'
...
class drawMosaic {
   ...
   * @return {}
   */
  draw(canvas: HTMLCanvasElement, image: HTMLCanvasElement | HTMLImageElement, drawList: Array<RENDERINFO> = []) {
    if (!canvas) return
    ...
    drawList.forEach(info => {
      ...
      positionInfo.forEach(item => {
        this.addMosaic(this.baseImageCtx as CanvasRenderingContext2D, ctx, ...item as [number, number, number])
      })
    })
  }
  /**
   ...
   * @param size 马赛克画笔大小
   */
  addMosaic(imageCtx: CanvasRenderingContext2D, ctx: CanvasRenderingContext2D, x: number, y: number, size: number) {
    // 当前位置绘制过马赛克不再绘制
   ...
   * @param colors 颜色数组
   */
  setAxisColor(imgData: ImageData, x: number, y: number, colors: Array<number>
  ) {
    const w = imgData.width
