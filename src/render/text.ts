...
import { TEXTPOSITIONINFO, TEXTRENDERINFO, OPTION, CUTBOXINFO } from '../types/index'
/**
 * @func 绘制文本
...
class drawText {
  ...
  baseFontSize: number
  minWidth: number
  showTextDom: HTMLElement | null
  cutBoxInfo: CUTBOXINFO
  ctx: CanvasRenderingContext2D | null
  opearteInfo: TEXTRENDERINFO | null
  constructor() {
   ...
   * @return {}
   */
  initData(ctx: CanvasRenderingContext2D, cutBoxInfo: CUTBOXINFO, isHaveEdit: boolean = false) {
    // 创建文本编辑dom
   ...
   * @return {}
   */
  draw(ctx: CanvasRenderingContext2D, info: TEXTRENDERINFO) {
    ...
    const { textConfig, width, height } = this.getTextConfig(
      ctx,
      positionInfo,
      this.calcFontSize(this.baseFontSize, option.size)
    )
    // 构造范围path,用于辅助判断事件处理
  ...
  // 设置样式配置
  setOptions(ctx: CanvasRenderingContext2D, option: OPTION, isEdit: boolean) {
    const { color, size = 1 } = option
    const fontSize = this.calcFontSize(this.baseFontSize, size)
    ctx.fillStyle = color
    ctx.strokeStyle = color
    ctx.lineWidth = 1
    ctx.font = `normal bold ${fontSize}px Bahnschrift`
    ctx.textBaseline = 'top'
    if (isEdit) {
      // 保持样式同步
      this.showTextDom!.style.fontSize = fontSize + 'px'
      this.showTextDom!.style.color = color
      this.showTextDom!.style.fontWeight = 'bold'
      this.showTextDom!.style.fontFamily = 'Bahnschrift'
      this.showTextDom!.style.boxShadow = `inset 0 0 0 1px ${color}`
    }
  }
  // 渲染每个文字的具体位置
  renderWrapText(ctx: CanvasRenderingContext2D, renderTextList: Array<any>) {
   ...
   * @return {}
   */
  drawAuxiliaryDom(positionInfo: TEXTPOSITIONINFO, cutBoxInfo: CUTBOXINFO) {
    const { x, y, text = '' } = positionInfo
    const {
      x: cutBoxX,
      y: cutBoxY,
  ...
  getTextConfig(context: CanvasRenderingContext2D, positionInfo: TEXTPOSITIONINFO, fontSize: number) {
    const { x, y, text = '', maxWidth = 0, maxHeight = 0 } = positionInfo
  ...
  drawTextLedge(ctx: Path2D, start: Array<number>, width: number, height: number) {
    const [x, y] = start
  ...
  addListener() {
    this.showTextDom!.oninput = e => {
      ...
      const { maxHeight = 0, text = '' } = this.opearteInfo.positionInfo
      const { innerText, clientHeight } = e.target as HTMLElement
      // 输入内容超出最大位置限制后
   ...
   * }
   */
  judgeOperate(info: TEXTRENDERINFO, ctx: CanvasRenderingContext2D, x: number, y: number) {
    const { canvasPath2D } = info
    if (!canvasPath2D) return
    if (ctx.isPointInPath(canvasPath2D, x * this.dpr, y * this.dpr)) {
      return {
        mouseCursor: 'move',
        fnExecution: (positionInfo: TEXTPOSITIONINFO, offsetX: number, offsetY: number) => {
   ...
   * @return {}
   */
  handleNewData(positionInfo: TEXTPOSITIONINFO) {
