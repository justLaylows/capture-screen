...
import generteCanvas from './common/generteCanvas' // 生成canvasdom
import drawCutBox from './common/cutBox' // 绘制剪切框
import ToolPanel from './common/toolPanel' // 创建工具栏
import ToolOption from './common/toolOption' // 创建配置栏

import renderArrowLine from './render/arrowLine' // 绘制箭头
import renderCircle from './render/circle' // 绘制椭圆
import renderBrush from './render/brush' // 绘制任意线
import renderRectangle from './render/rectangle' //绘制矩形
import renderMosaic from './render/mosaic' // 绘制马赛克
import renderText from './render/text' // 绘制文本

import drawingArrowLine from './drawing/arrowLine' // 收集绘制过程箭头信息
import drawingCircle from './drawing/circle' // 收集绘制过程椭圆信息
import drawingBrush from './drawing/brush' // 收集绘制过程画笔信息
import drawingRectangle from './drawing/rectangle' // 收集绘制过程矩形信息
import drawingMosaic from './drawing/mosaic' // 收集绘制过程马赛克信息
import drawingText from './drawing/text' // 收集绘制过程文本信息
import { fileDownload, savefile } from './common/fileMethod' // 文件操作方法
...
class captureScreen {
  ...
  constructor(
    ...
    this.initData(basicImage,isWebRtc, loadCallback)
  }
  async initData(basicImage:HTMLImageElement |null,isWebRtc: boolean, loadCallback: Function | null) {
    // 创建canvas画布
    const { canvasDom, ctx, imageCanvas, image } = await generteCanvas(
      this.captureDom as HTMLElement,
      this.drawWidth,
      this.drawHeight,
      basicImage,
      isWebRtc,
    ...
    this.image = image as HTMLCanvasElement | HTMLImageElement
    // 绘制底图
    renderMosaic.draw(this.imageCanvas as HTMLCanvasElement, this.image)
    // 创建剪裁框实例，并监听剪裁框事件
    this.generteCutBox(this.canvasDom as HTMLCanvasElement)
   ...
   * @return {}
   */
  generteCutBox(canvasDom: HTMLCanvasElement) {
    const { width, height } = canvasDom
    const callBack = (cutBoxInfo: CUTBOXINFO, type: string) => {
   ...
   * @return {}
   */
  renderGraph(isSave: boolean = false, selectId: number | undefined = undefined, editId: number | undefined = undefined) {
    if (!this.ctx) return
    // 依次先绘制底图，工具栏图形，剪裁框
    ...
    renderText.initData(this.ctx, this.cutBoxInfo as CUTBOXINFO, isHaveEdit.length > 0)
    ...
    this.toolData.forEach(item => {
      ...
      switch (item.type) {
        // 矩形
        case 'rectangle':
          item.canvasPath2D = renderRectangle.draw(this.ctx as CanvasRenderingContext2D, item)
          item.judgeOperate = renderRectangle.judgeOperate.bind(renderRectangle)
          break
        //椭圆
        case 'circle':
          item.canvasPath2D = renderCircle.draw(this.ctx as CanvasRenderingContext2D, item)
          item.judgeOperate = renderCircle.judgeOperate.bind(renderCircle)
          break
        //箭头
        case 'arrow':
          item.canvasPath2D = renderArrowLine.draw(this.ctx as CanvasRenderingContext2D, item)
          item.judgeOperate = renderArrowLine.judgeOperate.bind(renderArrowLine)
          break
        // 画笔
        case 'brush':
          item.canvasPath2D = renderBrush.draw(this.ctx as CanvasRenderingContext2D, item)
          item.judgeOperate = renderBrush.judgeOperate.bind(renderBrush)
          break
        // 文本 特殊处理
        case 'text':
          item.canvasPath2D = renderText.draw(this.ctx as CanvasRenderingContext2D, item)
   ...
   * @return {}
   */
  addListener(canvasDom: HTMLCanvasElement) {
  ...
  // 检查当前点击位置是否在其中某一个文本上
  checkInsideText(mouseX: number, mouseY: number) {
  ...
  handleMousedown(e: MouseEvent) {
    ...
    if (this.judgeResult && this.operationData) {
      ...
      this.startPoistion = JSON.parse(
        JSON.stringify('positionInfo' in this.operationData! ? this.operationData.positionInfo : this.operationData!)
      )
      // 渲染图形被选中的状态
      const id = 'id' in this.operationData! ? this.operationData!.id : undefined
      this.renderGraph(false, id)
      return
    }
    //当前是否为可绘制图形状态
    if (
   ...
   * @return {}
   */
  handleMouseMove(e: MouseEvent, dom: HTMLElement) {
    const mouseX = e.offsetX
    const mouseY = e.offsetY
    // 执行图形移动、拖拽
    if (this.operateStatus === 'edit') {
      ...
      const { fnExecution } = this.judgeResult as JUDGERESULT
      if (fnExecution) {
        ...
        Object.assign(
          'positionInfo' in this.operationData! ? this.operationData.positionInfo : this.operationData!,
          positionInfo
        )
      }
   ...
   * @return {}
   */
  handleMouseUp(e: MouseEvent, dom: HTMLElement) {
    // 操作移动为剪裁框时，更新工具栏位置状态
    this.operateStatus === 'edit' &&
      !this.toolData.length &&
      this.updateToolsBox()
    ...
    if (this.operateStatus === 'drawing' && this.drawingInfo) {
      ...
      // 如果是文本需要直接开启编辑模式
      const editId = this.activeTool === 'text' ? info.id : undefined
   ...
   * @return {}
   */
  handleDrawingEvent(e: MouseEvent, eventType = 'mouseDown') {
    switch (this.activeTool) {
      // 矩形
      case 'rectangle':
        if (eventType === 'mouseDown') {
          drawingRectangle.mouseDownEvent(e, this.cutBoxInfo as CUTBOXINFO)
        } else {
          // 清除画布
          this.renderGraph()
      ...
      case 'circle':
        if (eventType === 'mouseDown') {
          drawingCircle.mouseDownEvent(e, this.cutBoxInfo as CUTBOXINFO)
        } else {
          // 清除画布
          this.renderGraph()
      ...
      case 'arrow':
        if (eventType === 'mouseDown') {
          drawingArrowLine.mouseDownEvent(e, this.cutBoxInfo as CUTBOXINFO)
        } else {
          // 清除画布
          this.renderGraph()
      ...
      case 'brush':
        if (eventType === 'mouseDown') {
          drawingBrush.mouseDownEvent(e, this.cutBoxInfo as CUTBOXINFO)
        } else {
          // 清除画布
          this.renderGraph()
      ...
      case 'mosaic':
        ...
        if (eventType === 'mouseDown') {
          this.drawingInfo = drawingMosaic.mouseDownEvent(
            ...
            this.options,
            this.image as HTMLCanvasElement | HTMLImageElement
          )
        } else {
          this.drawingInfo = drawingMosaic.mouseMoveEvent(e, ctx as CanvasRenderingContext2D, this.options)
        }
        break
      // 文本 特殊处理
      case 'text':
   ...
   * }
   */
  correctOffset(start: Array<number>, move: Array<number>, cutBoxInfo: CUTBOXINFO) {
  ...
  updateToolsBox(isShow = true) {
    ...
    if (!this.toolsBox) {
      ...
      this.captureDom!.appendChild(this.toolsBox as HTMLElement)
    }
  ...
  // 处理工具栏点击事件
  toolBoxClickEvent(e: MouseEvent) {
    const target = e.target as HTMLElement
    if (!target.className.includes('capture_screen_tool_item')) return
    const domainTools = ['revoke', 'download', 'exit', 'save']
    const value = target.getAttribute('tool-data') as string
    // 功能性工具栏特殊处理
  ...
  toolFunction(type: string) {
    switch (type) {
      ...
      case 'revoke':
        ...
        renderMosaic.draw(this.imageCanvas as HTMLCanvasElement, this.image as HTMLCanvasElement | HTMLImageElement, mosaicList)
        // 重新渲染图形
        this.renderGraph()
        break
      // 下载
      case 'download':
        ...
        fileDownload(this.canvasDom as HTMLCanvasElement, this.cutBoxInfo as CUTBOXINFO)
        this.destory()
        break
      case 'exit':
        this.destory()
      ...
      case 'save':
        ...
        const dataURL = savefile(this.canvasDom as HTMLCanvasElement, this.cutBoxInfo as CUTBOXINFO)
  ...
  updateOptionsBox() {
    ...
    if (!this.optionBox) {
      ...
      this.toolsBox.appendChild(this.optionBox as HTMLElement)
    }

    ToolOption.setToolsPosition(this.toolsBox, this.optionBox, this.activeTool)
  }
  // 处理配置栏size点击事件
  optionSizeClick(e: MouseEvent) {
    const target = e.target as HTMLElement
    if (!target.className.includes('size_item')) return

    const value = target.getAttribute('size-data')
    ...
    target.classList.add('active')
    this.options.size = +(value || 0)
  }
  // 处理配置栏color点击事件
  optionColorClick(e: MouseEvent) {
    const target = e.target as HTMLElement
    if (!target.className.includes('color_item')) return

    const value = target.getAttribute('color-data') as string
  ...
  checkIsRectInside(mouseX: number, mouseY: number) {
    const { x, y, width, height } = this.cutBoxInfo as CUTBOXINFO
