...
class ToolOption {
   ...
   * @return {}
   */
  createOptionDom(sizeClickEvent:(e:MouseEvent)=>void, colorClcikEvent:(e:MouseEvent)=>void, defaultOption:OPTION) {
  ...
  createSizeDom(sizeClickEvent:(e:MouseEvent)=>void, defaultOption:OPTION) {
    const sizeList = [
      {
        value: 0.5,
      },
  ...
  createColorDom(colorClcikEvent:(e:MouseEvent)=>void, defaultOption:OPTION) {
    const colorList = [
      {
        value: "#ff0000",
      },
   ...
   * @return {}
   */
  setToolsPosition(toolDom:HTMLElement, optionDom:HTMLElement, activeTool:string) {
    const isShow = !!activeTool;
    optionDom.style.display = isShow ? "flex" : "none";
    // 设置颜色选择显示与隐藏状态，选中马赛克时不予展示颜色选择器
    const colorBox = optionDom.querySelector(".capture_screen_color-box") as HTMLElement;
    colorBox.style.display = activeTool === "mosaic" ? "none" : "flex";

    const activeToolDom = toolDom.querySelector(".active") as HTMLElement;
    // 用于设置角标位置
    const triangleDom = optionDom.querySelector(".triangle-icon") as HTMLElement;
