...
import { CUTBOXINFO, isValidKey } from '../types/index'
...
class drawCutBox {
  ...
  constructor(dom: HTMLElement, drawWidth: number, drawHeight: number, callback: Function) {
    ...
    this.dom = dom;
    this.drawWidth = drawWidth / dpr;
    this.drawHeight = drawHeight / dpr;
  ...
  // 通过事件监听获取剪裁框的位置信息
  drawing() {
    this.dom && this.dom.addEventListener("mousedown", this.handleMouseDown as EventListener);
  }
  destroy() {
    this.dom && (this.dom.style.cursor = "inherit");
    this.dom && this.dom.removeEventListener("mousedown", this.handleMouseDown as EventListener);
    this.dom && this.dom.removeEventListener("mousemove", this.handleMouseMove as EventListener);
    this.dom && this.dom.removeEventListener("mouseup", this.handleMouseUp as EventListener);
  }
  mouseDownEvent(e: MouseEvent) {
    ...
    this.dom.addEventListener("mousemove", this.handleMouseMove as EventListener);
    this.dom.addEventListener("mouseup", this.handleMouseUp as EventListener);
  }
   ...
   * @return {}
   */
  drawCutOutBox(ctx: CanvasRenderingContext2D, cutBoxInfo: CUTBOXINFO, isPoints: boolean = true) {
    ...
    isPoints &&
      pointsPlace.forEach((item) => {
        ctx.fillRect(...item as [number, number], borderSize, borderSize);
      });
  }
  // 绘制阴影蒙层
  drawShadowMask(ctx: CanvasRenderingContext2D, cutBoxInfo: CUTBOXINFO) {
    const { x, y, width, height } = cutBoxInfo;
    ...
    positionList.forEach((item) => {
      ctx.clearRect(...item as Parameters<typeof ctx.clearRect>);
      ctx.fillRect(...item as Parameters<typeof ctx.fillRect>);
    });
  }

  // 设置样式配置
  setOptions(ctx: CanvasRenderingContext2D) {
    const option = {
      strokeStyle: "#2CABFF",
      lineWidth: 1,
      fillStyle: "#2CABFF",
    ...
    Object.keys(option).forEach((key) => {
      isValidKey(key, ctx) && (ctx[key] = option[key])
    });
  }

  // 获取六个标识定位、顺时针
  getBorderPointsPlace(x: number, y: number, width: number, height: number, size: number) {
    return [
      [x - size / 2, y - size / 2],
   ...
   * }
   */
  judgeOperate(positionInfo: POSITIONINFO, mouseX: number, mouseY: number) {
    ...
    if (
      ...
      return {
        ...
        fnExecution: (positionInfo: POSITIONINFO, offsetX: number, offsetY: number) => {
          const { x, y, width, height } = positionInfo;
  ...
  // 判断点是否在矩形内
  checkIsRectInside(mouseX: number, mouseY: number, x: number, y: number, width: number, height: number) {
    return Math.abs(mouseX - x) <= width && Math.abs(mouseY - y) <= height;
  }
  // 处理新的位置数据信息
  handleNewData(newPosition: POSITIONINFO) {
