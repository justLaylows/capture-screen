...
// dataURL 转为 File
export const dataURLToFile = (fileDataURL:string, filename:string) => {
  let arr = fileDataURL.split(",") as Array<any>,
    mime = arr[0].match(/:(.*?);/)[1],
    bstr = atob(arr[1]),
    n = bstr.length,
 ...
 * @return {}
 */
export const fileDownload = (drawDoneCanvas:HTMLCanvasElement, cutBoxInfo:CUTBOXINFO) => {
 ...
 * @return {}
 */
export const savefile = (drawDoneCanvas:HTMLCanvasElement, cutBoxInfo:CUTBOXINFO) => {
  ...
  const ctx = imgContainer.getContext('2d') as CanvasRenderingContext2D
  // 剪裁图片
